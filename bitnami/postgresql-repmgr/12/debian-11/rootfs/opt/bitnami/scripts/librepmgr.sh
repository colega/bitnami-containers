...
repmgr_get_primary_node() {
    ...
    if [[ -f "$REPMGR_PRIMARY_ROLE_LOCK_FILE_NAME" ]]; then
        ...
        if [[ -z "$upstream_host" ]]; then
            info "Can not find new primary, since this node was the primary before, we'll assume that role again..."
            ...
            info "This node is the current primary, so we'll keep that role..."
            primary_host="$REPMGR_PRIMARY_HOST"
            primary_port="$REPMGR_PRIMARY_PORT"
        else
